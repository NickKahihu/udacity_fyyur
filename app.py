...
class Venue(db.Model):
    ...
    number_of_upcoming_shows = db.Column(db.Integer, default=0)
    number_of_past_shows=db.Column(db.Integer, default=0)
...
class Show(db.Model):
    ...
    venue_id = db.Column(db.Integer,db.ForeignKey('Venue.id')
                          ,nullable=False)
    upcoming_show=db.Column(db.Boolean, nullable=False, default=True)
...
def venues():
  ...
  #       num_upcoming_shows should be aggregated based on number of upcoming shows per venue.
    real_venue_areas=db.session.query(Venue.city,Venue.state).group_by(Venue.state,
    Venue.city).all()
    
    data=[]
    for area in real_venue_areas:
        venues = db.session.query(Venue.id,Venue.name,
          Venue.number_of_upcoming_shows).filter(Venue.city==area[0],Venue.state==area[1]).all()
    data.append({
        "city": area[0],
        "state": area[1],
        "venues": []
    ...
    for venue in venues:
        data[-1]["venues"].append({
              "id": venue[0],
              "name": venue[1],
              "number_of_upcoming_shows":venue[2]
      })
  ...
  # search for "band" should return "The Wild Sax Band".
    ...
    for artist in results:
        response['data'].append({
            "id": artist.id,
            "name": artist.name,
            "num_upcoming_shows": artist.number_of_upcoming_shows,
        })
